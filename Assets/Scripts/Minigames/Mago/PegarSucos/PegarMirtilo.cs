...
public class PegarMirtilo : MonoBehaviour
{
    public bool clicou;
    private MoverAteMenu moverAteMenu;
    public GameObject menu;
    public GameObject objeto;
    public static PegarMirtilo instance;
    ...
    void Clicou()
    {
        if(Input.GetMouseButtonDown(0)) 
        {
            ...
            {
                if(hit.collider.tag == "Mirtilo")
                {
                    gameObject.GetComponent<BoxCollider2D>().enabled = false;
                    moverAteMenu.irAteHUD = true;
                    clicou = true;
                    Inventario.instance.AdicionarItem(5);
                }
            }
        }
    }
    ...
    void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if(objeto.name == "Poção de Laranja" || objeto.name == "Poção de Morango")
        {
            objeto = null;
        }
    }
