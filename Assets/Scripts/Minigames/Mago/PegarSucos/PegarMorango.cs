...
public class PegarMorango : MonoBehaviour
{
    public bool clicou;
    private MoverAteMenu moverAteMenu;
    public GameObject menu;
    public GameObject objeto;
    public static PegarMorango instance;
    ...
    void Clicou()
    {
        if(Input.GetMouseButtonDown(0)) 
        {
            ...
            {
                if(hit.collider.tag == "Morango")
                {
                    gameObject.GetComponent<BoxCollider2D>().enabled = false;
                    moverAteMenu.irAteHUD = true;
                    clicou = true;
                    Inventario.instance.AdicionarItem(6);
                }
            }
        }
    }
    ...
    void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if(objeto.name == "Poção de Mirtilo" || objeto.name == "Poção de Laranja")
        {
            objeto = null;
        }
    }
